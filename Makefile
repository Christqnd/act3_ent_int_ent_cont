...
test-api:
	...
	docker run --rm --volume `pwd`:/opt/calc --network calc-test-api --env PYTHONPATH=/opt/calc --env BASE_URL=http://apiserver:5000/ -w /opt/calc calculator-app:latest pytest --cov --cov-report=xml:results/api_coverage.xml --cov-report=html:results/api_coverage --junit-xml=results/api_result.xml -m api || true
